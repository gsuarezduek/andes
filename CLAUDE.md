...
- **Modelo `CashMovement`** (migración `add_cash_movements`): `type` (`income`/`expense`), `description`, `amount`, vínculo **opcional** a `Rental`, y medio de pago como **snapshot** (`paymentMethodId` + `paymentMethodName`) — reusa el `PaymentMethod` ya existente de `/settings/payment-methods`; si el medio se edita/borra después, el movimiento cargado no se ve afectado (mismo criterio que `RentalPayment` en `src/lib/contract.ts`). `createdById`/`createdBy` para auditoría. **Inmutable**: sin edición ni borrado (un error se corrige con un movimiento inverso, no editando el original). Fecha = `createdAt` (momento de carga, no editable).
