...
## v7 — Notas del equipo por vehículo (con notificación en el Calendario)

Mensajería interna entre compañeros sobre un auto (distinta del campo libre `notes` de la ficha). Construida y probada en local (build/lint/tsc en verde; verificado por navegador en viewport móvil 375px y en el Calendario). **Falta desplegar la migración `add_vehicle_notes` en Railway.**

- **Modelo** `VehicleNote` (migración `add_vehicle_notes`): `text`, `createdById`/`createdBy` (quién la cargó), `resolvedById`/`resolvedBy` + `resolvedAt` (quién y cuándo la resolvió, nullable = activa). Cualquier rol puede cargar/resolver (es comunicación de equipo, no una acción de admin).
- **Ficha del vehículo** (`/vehicles/[id]`): sección **"Notas del equipo"** justo después del título y el badge de estado. Notas activas resaltadas en rojo con autor+fecha y botón **"Resolver"**; formulario para agregar; **historial** (`<details>`) con las resueltas (autor, fecha de carga, quién la resolvió y cuándo). Acciones en `notes-actions.ts` (`addVehicleNote`, `resolveVehicleNote`).
- **Calendario:** las notas sin resolver de un auto se traen en `getCalendarData` (`src/lib/calendar.ts`, `CalendarRow.activeNotes`) y se pintan como un **círculo rojo con el conteo** sobre la patente (columna sticky); al pasar el mouse, tooltip con el texto de cada nota + autor + fecha (reusa el mecanismo de hover ya existente para las barras, generalizado con un `HoverContent` discriminado bar/notes en `calendar-grid.tsx`). Al resolver la nota desde la ficha, desaparece del Calendario.
