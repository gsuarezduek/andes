...
- **Página `/caja`** (link nuevo en el nav principal, todos los roles): arriba, dos botones **"+ Cobro" / "+ Pago"** que abren el mismo formulario inline (detalle, monto, medio de pago obligatorio, reserva opcional — `<select>` con las reservas `reserved`/`active`, sin buscador nuevo). Server action única `createCashMovement` (`src/app/(app)/caja/actions.ts`, `requireUser`, cualquier rol puede cargar).
...
- Sin export CSV ni buscador de reservas nuevo (fuera de alcance, se puede sumar después reusando patrones existentes). Componentes en `src/components/cash/`.
