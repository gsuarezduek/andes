...
export const config = {
  ...
  matcher: [
    // `api/sync` y `api/daily-summary` se excluyen del proxy: los llama el
    // cron sin sesión y se autentican con CRON_SECRET dentro del route handler.
    ...
    "/((?!api/auth|api/sync|api/daily-summary|sign|api/sign|forgot-password|reset-password|_next/static|_next/image|favicon.ico|icon.svg|manifest.webmanifest|sw.js).*)",
  ],
};
